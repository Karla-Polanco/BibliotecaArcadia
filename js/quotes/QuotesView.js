...
export class QuotesView {
  ...
  render() {
    ...
    this.container.innerHTML = `
      ...
      <div class="quotes-header-panel" style="width: 100%; margin-bottom: 16px; padding: 16px; border-radius: var(--radius-md); background: linear-gradient(135deg, var(--color-surface), var(--color-surface-secondary)); border: 1px solid var(--color-border); border-left: 5px solid var(--color-primary-light);">
        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; margin-bottom: 12px;">
          ...
          <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
            <select id="select-filter-quotes" style="
              padding: 5px 10px;
              ...
              cursor: pointer;
            ">
              <option value="all" ${this.activeTab === 'all' ? 'selected' : ''}>Todas (${totalCount})</option>
              <option value="favorites" ${this.activeTab === 'favorites' ? 'selected' : ''}>Favoritas (${favCount})</option>
            </select>
      ...
      <div class="quotes-cards-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 12px; width: 100%;">
        ${filtered.length === 0 ? `
  ...
  renderQuoteCard(item) {
    ...
    return `
      ...
      ">
        ...
        <span style="position: absolute; right: 12px; top: 8px; font-size: 3rem; line-height: 1; font-family: Georgia, serif; color: var(--color-primary-light); opacity: 0.08; pointer-events: none; user-select: none;">“</span>
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
          <div style="display: flex; align-items: center; gap: 8px;">
            <button class="btn-toggle-fav-quote" data-action="toggle-fav" data-id="${item.id}" title="${item.favorite ? 'Quitar de favoritas' : 'Marcar como favorita'}" style="
              background: none; border: none; cursor: pointer; padding: 2px; color: ${item.favorite ? '#F59E0B' : 'var(--color-text-muted)'}; display: flex; align-items: center; font-size: 1rem;
            ">
              ${item.favorite ? '★' : '☆'}
            </button>
            <span style="font-size: 0.7rem; color: var(--color-text-muted); font-weight: 500;">${dateStr}</span>
          </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 2px;">
          <span style="font-size: 0.8rem; font-weight: bold; color: var(--color-text); letter-spacing: 0.03em; text-transform: uppercase;">— ${this.escapeHtml(item.author || 'Anónimo')}</span>
          ${item.source ? `<span style="font-size: 0.74rem; color: var(--color-text-muted); font-style: italic;">${this.escapeHtml(item.source)}</span>` : ''}
        </div>

        <!-- Acciones Inferiores -->
        <div style="display: flex; align-items: center; justify-content: flex-end; border-top: 1px solid var(--color-border); padding-top: 8px; margin-top: auto; gap: 8px;">
          <button class="btn-copy-quote" data-action="copy" data-text="${escAttr(item.text)}" data-author="${escAttr(item.author)}" style="
            background: none;
            border: none;
  ...
  attachEvents() {
    ...
    // 2. Filtro por select
    const quotesFilter = this.container.querySelector('#select-filter-quotes');
    if (quotesFilter) {
      quotesFilter.addEventListener('change', (e) => {
        this.activeTab = e.target.value;
        this.render();
      });
      CustomSelect.enhance(quotesFilter);
    }
